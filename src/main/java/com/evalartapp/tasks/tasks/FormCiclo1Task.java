...
public class FormCiclo1Task implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                Enter.theValue(leerExcel.get(0).get("numero1")).into(INPUT_NUMERO),
                Enter.theValue(leerExcel.get(0).get("caracteres1")).into(TEXTAREA_TEXTO),
                Click.on(BTN_ENVIAR)
        );
    }
    public static FormCiclo1Task formCiclo1Task() {
        return instrumented(FormCiclo1Task.class);
    }
}
