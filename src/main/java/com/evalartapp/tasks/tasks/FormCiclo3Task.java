...
public class FormCiclo3Task implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                Enter.theValue(leerExcel.get(0).get("numero3")).into(INPUT_NUMERO),
                Enter.theValue(leerExcel.get(0).get("caracteres3")).into(TEXTAREA_TEXTO),
                Click.on(BTN_ENVIAR)
        );
    }
    public static FormCiclo3Task formCiclo3Task() {
        return instrumented(FormCiclo3Task.class);
    }
}
