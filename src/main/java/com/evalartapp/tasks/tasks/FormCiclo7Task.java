...
public class FormCiclo7Task implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                Enter.theValue(leerExcel.get(0).get("numero7")).into(INPUT_NUMERO),
                Enter.theValue(leerExcel.get(0).get("caracteres7")).into(TEXTAREA_TEXTO),
                Click.on(BTN_ENVIAR)
        );
    }
    public static FormCiclo7Task formCiclo7Task() {
        return instrumented(FormCiclo7Task.class);
    }
}
