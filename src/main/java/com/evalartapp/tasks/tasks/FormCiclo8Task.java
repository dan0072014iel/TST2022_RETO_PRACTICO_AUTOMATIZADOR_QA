...
public class FormCiclo8Task implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        ...
        actor.attemptsTo(
                Enter.theValue(leerExcel.get(0).get("numero8")).into(INPUT_NUMERO),
                Enter.theValue(leerExcel.get(0).get("caracteres8")).into(TEXTAREA_TEXTO),
                Click.on(BTN_ENVIAR)
        );
    }
    public static FormCiclo8Task formCiclo8Task() {
        return instrumented(FormCiclo8Task.class);
    }
}
